package com.ascend.flockr.client.spark.impl;
...
import com.ascend.flockr.config.SparkConfig;
import com.ascend.flockr.exception.spark.SparkApiException;
import com.ascend.flockr.exception.spark.SparkConnectionException;
import com.ascend.flockr.exception.spark.SparkJobNotFoundException;
import com.ascend.flockr.exception.spark.SparkJobSubmissionException;
...
public class SparkClientImpl implements SparkClient {
        ...
        return executeRequest(
                ...
                .map(
                        response -> {
                            ...
                            return SparkJobSubmissionResponse.builder()
    ...
    public Single<SparkJobStatusResponse> getJobStatus(String submissionId) {
        ...
        return executeRequest(
                ...
                .doOnSuccess(
                        status -> {
                            log.debug("Submission {} status: {}, ApplicationId: {}", 
                                    submissionId, status.getStatus(), status.getApplicationId());
                        })
                .doOnError(
                        error -> {
    ...
    public Completable cancelJob(String submissionId) {
        ...
        return executeRequest(
                HttpMethod.POST,
                "/v1/submissions/kill/" + submissionId,
                null,
    ...
    public Single<String> getApplicationLogs(String applicationId) {
        ...
        return executeRequest(
                ...
                .doOnError(
                        error -> {
                            log.error("Failed to get application logs for applicationId {}", applicationId, error);
                        }
        );
    }
    ...
    private Single<HttpResponse<Buffer>> executeRequest(
        ...
        if (method == HttpMethod.GET) {
            request =
                    webClient.prepareHttpGETRequest(
                            sparkConfig.getHost(), sparkConfig.getPort(), path);
        } else if (method == HttpMethod.POST) {
            request =
                    webClient.prepareHttpPOSTRequest(
                            sparkConfig.getHost(), sparkConfig.getPort(), path);
        } else {
    ...
    private Single<HttpResponse<Buffer>> validateResponse(
        ...
        if (response.statusCode() >= 200 && response.statusCode() < 300) {
            ...
            if (statusCode == 404) {
                return Single.error(new SparkJobNotFoundException(extractApplicationIdFromError(responseBody)));
