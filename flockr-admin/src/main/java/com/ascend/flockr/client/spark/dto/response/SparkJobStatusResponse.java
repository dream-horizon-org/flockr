 ...
 * 
 * <p>Contains job status information including applicationId, status, and other details
 * returned from Spark API when querying job status using submissionId.
 */
@Data
@Builder
@NoArgsConstructor
...
public class SparkJobStatusResponse {
   ...
   * 
   * <p>This ID is different from submissionId and is used to retrieve application logs.
   * Obtained from getJobStatus() response.
   */
   ...
   * 
   * <p>May contain error messages if the job failed, or informational messages
   * about job progress.
   */
