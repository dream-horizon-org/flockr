 ...
 *
 * <p>This class represents the configuration for one data source in the engine. The engine supports
 * only a single source per execution, but this configuration can be used for different source types
 * (Athena, S3, Kafka, etc.).
 *
 * <p><b>Example JSON:</b>
 *
...
public class SourceConfig {
   ...
   *
   * <p>For Athena sources, this is the SQL query that will be executed. For other source types,
   * this may be used differently or ignored.
   *
   ...
   *
   * <ul>
   *   <li><b>ATHENA</b>: AWS Athena data source
   *   <li><b>S3</b>: S3 data source
   *   <li><b>KAFKA</b>: Kafka data source
   * </ul>
   *
   ...
   *
   * <p>This map contains configuration parameters specific to the source type. The structure
   ...
   *   <li>sessionToken: Optional AWS session token for temporary credentials
   *   <li>database: Optional database name (can be extracted from query)
   * </ul>
   *
