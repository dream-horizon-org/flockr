 ...
 *
 * <ul>
 *   <li>Extracting AWS credentials from Athena or S3 source configurations
 *   <li>Setting system properties for S3A filesystem access
 *   <li>Configuring appropriate credentials provider based on session token presence
 * </ul>
 *
 * <p><b>Supported Source Types:</b>
 *
 * <ul>
 *   <li><b>ATHENA</b>: Extracts credentials from AthenaConfig (required)
 *   <li><b>S3</b>: Extracts credentials from S3Config (optional)
 * </ul>
 *
...
public class AwsCredentialsConfigurator {
   ...
   *
   * <ol>
   *   <li>Extracts the source type and config object
   *   <li>Configures AWS credentials based on source type
   *   <li>Sets system properties for S3A filesystem access
   * </ol>
   *
  ...
  public AthenaConfig configureCredentials(ConnectorConfig sourceConnectorConfig) {
    ...
    if ("ATHENA".equalsIgnoreCase(sourceType)) {
      ...
      log.info(
          "Athena config loaded - database: {}, region: {}",
  ...
  private void configureSystemProperties(
      String accessKey, String secretKey, String sessionToken, String sourceName) {
    ...
    if (sessionToken != null && !sessionToken.isEmpty()) {
      ...
      log.info(
          "AWS session token configured from {} - using TemporaryAWSCredentialsProvider",
          sourceName);
    } else {
      System.setProperty(
          "fs.s3a.aws.credentials.provider",
