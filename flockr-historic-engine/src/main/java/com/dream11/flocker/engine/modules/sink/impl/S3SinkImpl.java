 ...
 * 
 * <p>This class writes Spark datasets to S3 in various formats (Parquet, CSV, JSON)
 * with support for:
 * <ul>
 *   <li>Multiple write modes (append, overwrite, etc.)</li>
 *   <li>Compression (snappy, gzip, etc.)</li>
 *   <li>Custom partitioning</li>
 *   <li>Additional Spark options</li>
 ...
 * <p><b>Write Modes:</b>
 * <ul>
 *   <li>append: Add data to existing files</li>
 *   <li>overwrite: Replace existing data (default)</li>
 *   <li>error: Fail if data already exists</li>
 *   <li>ignore: Do nothing if data already exists</li>
 * </ul>
 * 
 * <p><b>Credentials:</b>
 * <p>The sink configures Hadoop/Spark S3A filesystem settings with AWS credentials
 * from the S3Config. Supports both permanent and temporary credentials (with session tokens).
 * 
 * <p><b>Individual Writes:</b>
 * <p>Individual write operations (write(String)) are supported by converting
 * the JSON string to a dataset and writing it.
 * 
 * @see Sink
 * @see S3Config
...
public class S3SinkImpl implements Sink<String> {
     ...
     * 
     * <p>If writeMode is null, uses the write mode from sinkConfig, or defaults to overwrite.
     * If outputPath is null, uses the S3 path from sinkConfig.
     * 
     * <p>Configures Hadoop S3A filesystem settings with AWS credentials from sinkConfig.
     * 
     * @param sinkConfig S3 configuration (must not be null).
     * @param sparkSession Spark session (must not be null).
     * @param writeMode Write mode (append, overwrite, etc.). If null, uses config or defaults to overwrite.
