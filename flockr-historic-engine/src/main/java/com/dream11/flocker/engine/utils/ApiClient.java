...
import com.dream11.flocker.engine.dto.AudienceUpdateRequest;
import com.dream11.flocker.engine.dto.builder.AudienceUpdateRequestFactory;
import com.dream11.flocker.engine.mapper.JsonMapper;
 ...
 * <ul>
 *   <li><b>Rate Limiting</b>: Enforces requests per second limit via {@link RateLimiter}</li>
 *   <li><b>Batching</b>: Splits large requests into smaller batches</li>
 *   <li><b>JSON Serialization</b>: Uses Jackson for type-safe JSON serialization</li>
 *   <li><b>Custom Headers</b>: Supports project key and content type headers</li>
 * </ul>
 *
 * <p><b>Usage:</b>
 * <pre>{@code
 ...
 *
 * <p><b>Rate Limiting:</b>
 * <p>The client uses a {@link RateLimiter} to enforce rate limiting by tracking
 * the time between requests and sleeping if necessary to maintain the configured
 * requests per second limit.
 *
 * <p><b>Batching:</b>
 * <p>Large lists of user IDs are automatically split into batches based on
 * the configured batch size. Each batch is sent as a separate HTTP request.
 *
 * <p><b>Request Format:</b>
 * <p>The client sends POST requests with JSON payloads serialized from
 * {@link AudienceUpdateRequest} POJOs in the following format:
 * <pre>{@code
 * [
 *   {
 *     "user_id": 12345,
...
public class ApiClient {
     ...
     *
     * <p>This constructor creates a client without API configuration, which
     * will simulate API calls (log only) when {@link #notifyAudienceUpdate} is called.
     *
     * <p><b>Note:</b> This is primarily for testing. Use {@link #ApiClient(ApiConfig)}
     * for production use.
     */
    public ApiClient() {
        this.apiConfig = null;
     ...
     *
     * <p>This method sends user IDs to the configured API endpoint in batches,
     * with rate limiting applied between batches. The request payload includes
     * user IDs, cohort key (audience name), action, and expiration timestamp.
     *
     * <p><b>Batching:</b>
     * <p>If the number of user IDs exceeds the configured batch size, they are
     * split into multiple batches. Each batch is sent as a separate HTTP request.
     *
     * <p><b>Rate Limiting:</b>
     * <p>Between each batch request, the client waits if necessary to maintain
     ...
     */
    public void notifyAudienceUpdate(String audienceName, List<String> userIds, String action, Long expireAt) {
        if (apiConfig == null) {
     ...
     * <ul>
     *   <li>Content-Type header from configuration</li>
     *   <li>x-project-key header if configured</li>
     *   <li>JSON body serialized from {@link AudienceUpdateRequest} POJOs</li>
     * </ul>
     *
     * @param audienceName The audience name (used as audience_key) - comes from application arguments.
     ...
     */
    private void makeApiCall(String audienceName, List<String> userIds, String action, Long expireAt, int batchNumber, int totalBatches) throws Exception {
     ...
     *
     * <p>This method converts user IDs to {@link AudienceUpdateRequest} POJOs
     * and serializes them to JSON using Jackson's ObjectMapper.
     *
     * <p>The request body is a JSON array of objects, each containing:
     * <ul>
     *   <li>user_id: The user ID (number or string, automatically detected)</li>
     *   <li>audience_key: The audience name (maps to audienceName from application arguments)</li>
     *   <li>action: The action ("append" or "remove")</li>
     *   <li>expire_at: The expiration timestamp</li>
     * </ul>
     *
     * @param audienceName The audience name (used as audience_key) - comes from application arguments.
     ...
     */
    private String buildRequestBody(String audienceName, List<String> userIds, String action, Long expireAt) {
        try {
            List<AudienceUpdateRequest> requests = userIds.stream()
                    .map(userId -> AudienceUpdateRequestFactory.fromString(userId, audienceName, action, expireAt))
