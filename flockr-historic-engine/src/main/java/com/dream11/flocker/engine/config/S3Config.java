 ...
 * 
 * <p>This class contains all configuration parameters needed to read from or
 * write to Amazon S3, including bucket, path, format, compression, and AWS credentials.
 * 
 * <p><b>Usage:</b>
 * <p>This configuration can be used for both sources and sinks:
 * <ul>
 *   <li><b>Source</b>: Read data from S3 (Parquet, CSV, etc.)</li>
 *   <li><b>Sink</b>: Write data to S3 (Parquet, CSV, etc.)</li>
 * </ul>
 * 
 * <p><b>Required Fields (for sink):</b>
 * <ul>
 *   <li>bucket: S3 bucket name</li>
 *   <li>path: S3 path within the bucket</li>
 * </ul>
 * 
 * <p><b>Optional Fields:</b>
 * <ul>
 *   <li>format: File format (PARQUET, CSV, JSON) - defaults to PARQUET</li>
 *   <li>compression: Compression type (snappy, gzip, etc.)</li>
 *   <li>accessKey: AWS access key ID (inherited from source if not provided)</li>
 *   <li>secretKey: AWS secret access key (inherited from source if not provided)</li>
 *   <li>sessionToken: AWS session token for temporary credentials</li>
 *   <li>region: AWS region</li>
 *   <li>partitions: Number of partitions for writing (0 = auto)</li>
 *   <li>writeMode: Write mode (append, overwrite, etc.) - defaults to append</li>
 *   <li>options: Additional Spark options as key-value pairs</li>
 * </ul>
 * 
...
public class S3Config {
     ...
     * {@code config/source/s3/default.conf}
     * 
     ...
     * 
     * <p>This provider loads configuration from the default config file location:
     * {@code config/sink/s3/default.conf}
     * 
     * @return A ConfigProvider instance for S3Config.
     */
     ...
     * <ul>
     *   <li>fs.s3a.access.key: AWS access key</li>
     *   <li>fs.s3a.secret.key: AWS secret key</li>
     *   <li>fs.s3a.session.token: AWS session token (if provided)</li>
     *   <li>fs.s3a.aws.credentials.provider: Credentials provider class</li>
     *   <li>fs.s3a.region: AWS region (if provided)</li>
     * </ul>
     * 
     * <p>If a session token is provided, it uses TemporaryAWSCredentialsProvider,
     * otherwise it uses SimpleAWSCredentialsProvider.
     * 
     ...
     * 
     * <p>If the path already starts with "s3://" or "s3a://", it is returned as-is.
     * Otherwise, it constructs the path as "s3a://{bucket}/{path}".
     * 
     ...
     * 
     * <p>Returns the format name in lowercase (e.g., "parquet", "csv", "json").
     * Defaults to "parquet" if format is not specified.
     * 
     ...
     * including optional fields like format, compression, partitions, and options.
     * 
