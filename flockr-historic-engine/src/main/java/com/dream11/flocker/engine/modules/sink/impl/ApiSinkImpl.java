 ...
 * <ul>
 *   <li>Collecting user IDs from the dataset</li>
 *   <li>Sending them to the API via ApiClient with rate limiting and batching</li>
 *   <li>Including audience metadata (audience_key, action, expire_at) in requests</li>
 * </ul>
 * 
 * <p><b>Data Format:</b>
 * <p>The sink extracts user IDs from the first column of the dataset and sends
 * them to the API along with:
 * <ul>
 ...
 * 
 * <p><b>Rate Limiting:</b>
 * <p>The ApiClient handles rate limiting and batching automatically based on
 * the ApiConfig settings.
 * 
 * <p><b>Limitations:</b>
 * <p>This sink does not support individual write operations (throws
 * UnsupportedOperationException). It only supports dataset-level writes.
 * 
 * <p><b>Memory Considerations:</b>
 * <p>The sink collects all user IDs to the driver node using collectAsList(),
 * which may cause memory issues with very large datasets. Consider this when
 * processing extremely large cohorts.
 * 
 * @see Sink
 * @see ApiConfig
