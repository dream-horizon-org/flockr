package com.dream11.flocker.engine.dto.builder;
 ...
 *
 * <p>This factory provides methods to create audience update requests with different
 * user ID types (string or numeric). It handles automatic type conversion and
 ...
 * <p>This factory can be extended to support additional API request types as needed.
 * For example, if new request types are added (e.g., AudienceDeleteRequest,
 * AudienceQueryRequest), similar factory methods can be added here or in separate
 * factory classes following the same pattern.
 *
...
public class AudienceUpdateRequestFactory {
     ...
     */
    public static AudienceUpdateRequest withStringUserId(String userId, String audienceName, String action, Long expireAt) {
     ...
     * @return A new AudienceUpdateRequest instance.
     */
    public static AudienceUpdateRequest withNumericUserId(Long userId, String audienceName, String action, Long expireAt) {
        return AudienceUpdateRequest.builder()
                .userId(userId)
                .audienceKey(audienceName)
                .action(action != null ? action : "append")
    ...
    /**
     * Creates an AudienceUpdateRequest from a string user ID, automatically
     * converting to numeric if possible.
     *
     * <p>This method attempts to parse the string as a long. If successful,
     * it creates a request with a numeric user ID. Otherwise, it creates
     * a request with a string user ID.
     *
     * @param userIdStr The user ID as a string.
     * @param audienceName The audience name (maps to audience_key) - comes from application arguments.
     * @param action The action ("append" or "remove").
     * @param expireAt The expiration timestamp in epoch seconds.
     * @return A new AudienceUpdateRequest instance with numeric or string user ID.
     */
    public static AudienceUpdateRequest fromString(String userIdStr, String audienceName, String action, Long expireAt) {
        try {
