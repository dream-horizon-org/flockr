 ...
 * <ul>
 *   <li>Execute SQL queries on AWS Athena</li>
 *   <li>Wait for query completion with polling</li>
 *   <li>Read query results from S3 (CSV or Parquet format)</li>
 *   <li>Handle AWS credentials (permanent and temporary with session tokens)</li>
 *   <li>Extract database name from SQL query if not provided in config</li>
 * </ul>
 * 
 * <p><b>Query Execution Flow:</b>
 * <ol>
 *   <li>Validates AWS credentials</li>
 *   <li>Creates Athena client with credentials</li>
 *   <li>Starts query execution via Athena API</li>
 *   <li>Polls query status until completion (max 5 minutes)</li>
 *   <li>Reads results from S3 output location</li>
 * </ol>
 * 
 * <p><b>Result Reading:</b>
 * <p>The class attempts to read results as CSV first (Athena's default format),
 * then falls back to Parquet if CSV reading fails. Results are read from the
 * S3 location specified in the configuration.
 * 
 * <p><b>Database Extraction:</b>
 * <p>If database is not provided in config, the class attempts to extract it from
 * the SQL query by looking for patterns like "FROM database.table" or "JOIN database.table".
 * 
 * <p><b>Credentials:</b>
 * <p>Supports both permanent credentials (accessKey + secretKey) and temporary
 * credentials (accessKey + secretKey + sessionToken). The appropriate credentials
 * provider is configured automatically.
 * 
 * @see Source
 * @see AthenaConfig
...
public class AthenaSourceImpl implements Source<Dataset<Row>> {
    ...
    private QueryExecutionResult executeAthenaQuery(String sqlQuery) throws Exception {
        ...
        // Extract database from query if not provided in config (e.g., "SELECT * FROM database.table" or "FROM 'database'.'table'")
        String database = athenaConfig.getDatabase();
        if ((database == null || database.trim().isEmpty()) && athenaConfig.getSqlQuery() != null) {
            // Try to extract database from query (format: database.table, 'database'.'table', or "database"."table")
            String query = athenaConfig.getSqlQuery().trim();
            // Look for patterns like: FROM database.table, FROM 'database'.'table', JOIN "database"."table", etc.
