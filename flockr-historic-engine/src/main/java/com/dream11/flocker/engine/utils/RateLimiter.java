 ...
 * 
 * <p>This class tracks the time between requests and sleeps if necessary to
 * maintain the configured rate limit. It uses atomic operations to ensure
 * thread safety when used by multiple threads.
 * 
 * <p><b>Usage:</b>
 * <pre>{@code
...
public class RateLimiter {
    ...
    public RateLimiter(int requestsPerSecond) {
        ...
        log.debug("RateLimiter initialized with {} requests/sec (min interval: {}ms)",
                requestsPerSecond, minIntervalMillis);
    }

    /**
     * Acquires permission to make a request, waiting if necessary to maintain the rate limit.
     * 
     * <p>This method calculates the time since the last request and sleeps
     * if necessary to maintain the configured requests per second limit.
     * 
     * <p><b>Thread Safety:</b>
