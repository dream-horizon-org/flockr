 ...
 *
 * <p>For Athena source, the SQL query is already executed by the source, so the results are used
 * directly. For other source types, the SQL query may be applied to the read data.
 *
 * <p><b>Sink Writing:</b>
 *
 * <p>The service attempts to write using {@link Sink#writeDataset(Dataset)} first (preferred for
 * distributed processing). If not supported, it falls back to collecting rows and writing
 * individually (may cause memory issues with large datasets).
 *
 * @see Source
 * @see Sink
...
public class AudienceProcess {
   ...
   *
   * <ul>
   *   <li>user_id: Long or String (from first column of query result)
   *   <li>audience_key: String (set to audienceName from application arguments)
   *   <li>action: String ("append" or "remove")
   *   <li>expire_at: Long (epoch timestamp in seconds)
   * </ul>
   *
  ...
  public void processWithQuery(String sqlQuery, String audienceName, String action, Long expireAt) {
    try {
      ...
      if (source instanceof AthenaSourceImpl) {
        log.info(
            "Athena source detected - query already executed on Athena, using results directly");
        queryResult = data;
      } else {
      ...
      // Check if the first column is a valid user ID column (userId, userid, or user_id,
      // case-insensitive)
