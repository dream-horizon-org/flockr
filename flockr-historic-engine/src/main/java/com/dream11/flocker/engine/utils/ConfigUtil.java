...
public final class ConfigUtil {
  ...
  public static Config getConfigFromConfigFile(@NonNull String configFilePathFormat) {
    ...
    Config envConfig =
        ConfigFactory.parseResources(envFile, ConfigParseOptions.defaults().setAllowMissing(true));
    Config defaultConfig =
        ConfigFactory.parseResources(
            defaultFile, ConfigParseOptions.defaults().setAllowMissing(true));
  ...
  public static <T> T getTypedConfigFromConfigFile(
    ...
    try {
      ...
      throw new RuntimeException(
          "Failed to create config object of type "
              + clazz.getName()
              + ". Ensure the class has a static fromConfig(Config) method.",
          e);
    }
  }
}
