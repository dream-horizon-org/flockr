...
import com.dream11.flocker.engine.modules.sink.impl.KafkaSinkImpl;
 ...
 * <ul>
 *   <li><b>Source</b>: Provides a single Source instance based on source configuration</li>
 *   <li><b>Sinks</b>: Provides a list of Sink instances based on sink configurations</li>
 *   <li><b>SparkSession</b>: Binds the Spark session as a singleton</li>
 * </ul>
 * 
 * <p><b>Source Creation:</b>
 * <p>The module creates Source instances using the {@link com.dream11.flocker.engine.modules.source.impl.SourceFactory}
 * based on the source type (ATHENA, S3, KAFKA, etc.).
 * 
 * <p><b>Sink Creation:</b>
 * <p>The module creates Sink instances directly:
 * <ul>
 *   <li><b>S3</b>: Creates S3SinkImpl with configured S3 path and write mode</li>
 *   <li><b>API</b>: Creates ApiSinkImpl with API configuration and audience metadata</li>
 *   <li><b>KAFKA</b>: Currently not implemented (logs warning and skips)</li>
 * </ul>
 * 
 * <p><b>Configuration Handling:</b>
 * <p>The module handles both typed config objects (e.g., S3Config, ApiConfig) and
 * raw Typesafe Config objects, converting them as needed.
 * 
...
public class EngineModule extends AbstractModule {
    ...
    public EngineModule(SparkSession sparkSession,
            ...
            String audienceName, String action, Long expireAt) {
     ...
     * <p>This method:
     * <ol>
     *   <li>Converts raw Config objects to typed config objects if needed</li>
     *   <li>Creates Source instance using SourceFactory</li>
     *   <li>Returns the source as a singleton</li>
     * </ol>
     * 
     * <p><b>Supported Source Types:</b>
     * <ul>
     *   <li>ATHENA → AthenaSourceImpl</li>
     *   <li>S3 → S3SourceImpl</li>
     *   <li>KAFKA → KafkaSourceImpl (if implemented)</li>
     * </ul>
     * 
     ...
     * <p>This method:
     * <ol>
     *   <li>Iterates through sink configurations</li>
     *   <li>Converts raw Config objects to typed config objects if needed</li>
     *   <li>Creates Sink instances based on sink type</li>
     *   <li>Returns the list of sinks as a singleton</li>
     * </ol>
     * 
     * <p><b>Supported Sink Types:</b>
     * <ul>
     *   <li><b>S3</b>: Creates S3SinkImpl with S3 path and write mode</li>
     *   <li><b>API</b>: Creates ApiSinkImpl with API config and audience metadata</li>
     *   <li><b>KAFKA</b>: Currently not implemented (logs warning and skips)</li>
     * </ul>
     * 
     * <p><b>S3 Sink Configuration:</b>
     * <p>The S3 sink uses the configured bucket and path to construct the S3 URI.
     * Write mode defaults to "append" if not specified.
     * 
     * <p><b>API Sink Configuration:</b>
     * <p>The API sink is configured with the API config, audience name, action,
     * and expiration timestamp for building request payloads.
     * 
    ...
    public List<Sink<String>> provideSinks() {
        ...
        for (ConnectorConfig config : sinkConfigs) {
            ...
            if ("S3".equals(type)) {
                ...
                producerConfig.setValueSerializerClass("org.apache.kafka.common.serialization.StringSerializer");
                ...
                props.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, producerConfig.getValueSerializerClass());
                ...
                log.info("Added Kafka sink for topic: {} at bootstrap servers: {}", 
                    producerConfig.getTopic(), producerConfig.getBootstrapServers());
            }
        }
