...
public class SparkConfig {
    ...
    return new ConfigProvider<>("spark", SparkConfig.class) {
      ...
      protected String getConfigPath() {
        return String.format(
            "config/spark/%s.conf", com.dream11.flocker.engine.constants.Constants.DEFAULT_APP_ENV);
      }
    };
  }
  ...
  public static SparkConfig fromConfig(Config config) {
    ...
    sparkConfig.setS3aConnectionEstablishTimeout(
        getStringOrNull(config, "s3a.connection.establish.timeout"));
    sparkConfig.setS3aConnectionRequestTimeout(
        getStringOrNull(config, "s3a.connection.request.timeout"));
    ...
    sparkConfig.setSqlAdaptiveCoalescePartitionsEnabled(
        getBooleanOrNull(config, "sql.adaptive.coalescePartitions.enabled"));
  ...
  public void applyToSessionBuilder(org.apache.spark.sql.SparkSession.Builder builder) {
    ...
    if (s3aConnectionEstablishTimeout != null) {
      builder.config(
          "spark.hadoop.fs.s3a.connection.establish.timeout", s3aConnectionEstablishTimeout);
    }
    ...
    if (sqlAdaptiveCoalescePartitionsEnabled != null) {
      builder.config(
          ...
          String.valueOf(sqlAdaptiveCoalescePartitionsEnabled));
    }
